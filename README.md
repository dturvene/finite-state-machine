...
deterministic finite state machine (DFSM or just FSM).  The motivation for
...
to send events to/from the FSMs in order to perform functions to fulfill a
real-world objective. See the [fsm example](#fsm-example) for an illustration
of this.

An DFSM is essentially a set of states, each accepting a subset of all FSM
...
FSM Requirements
----------------
The FSM structure must be simple to conprehend.
